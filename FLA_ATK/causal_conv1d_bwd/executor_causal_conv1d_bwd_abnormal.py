...
# 异常用例约束说明（对应 atk_causal_conv1d_bwd_abnormal.json）。
# 以下为各用例设计时违反的输入约束；固定输出标杆不执行约束校验。
# 实际 DUT 的拒绝行为及报错需由运行结果确认。
# case 00: causal_conv1d_bwd_abnormal_x_rank_1
#   变异输入：x.shape = [1536]
#   违反约束：BSND x must be rank 3
# case 01: causal_conv1d_bwd_abnormal_x_scalar
#   变异输入：x.shape = []
#   违反约束：BSND x must be rank 3
# case 02: causal_conv1d_bwd_abnormal_weight_rank_1
#   变异输入：weight.shape = [1536]
#   违反约束：weight must be rank 2
# case 03: causal_conv1d_bwd_abnormal_weight_dim_mismatch
#   变异输入：weight.shape = [4, 1520]
#   违反约束：weight D must match x
# case 04: causal_conv1d_bwd_abnormal_dy_time_mismatch
#   变异输入：dy.shape = [1, 32767, 1536]
#   违反约束：dy token shape must match x
# case 05: causal_conv1d_bwd_abnormal_dy_rank_2
#   变异输入：dy.shape = [32768, 1536]
#   违反约束：BSND dy must be rank 3
# case 06: causal_conv1d_bwd_abnormal_x_int32
#   变异输入：x.dtype = "int32"
#   违反约束：x supports floating-point types only
# case 07: causal_conv1d_bwd_abnormal_weight_dtype_mismatch
#   变异输入：weight.dtype = "fp16"
#   违反约束：weight dtype must match BF16 x
# case 08: causal_conv1d_bwd_abnormal_activation_3
#   变异输入：activation.range_values = 3
#   违反约束：activation supports 0/1/2
# case 09: causal_conv1d_bwd_abnormal_activation_without_y
#   变异输入：activation.range_values = 1
#   违反约束：activation 1/2 requires y
# case 10: causal_conv1d_bwd_abnormal_layout_invalid
#   变异输入：inputLayoutOptional.range_values = "INVALID"
#   违反约束：invalid layout
# case 11: causal_conv1d_bwd_abnormal_initial_wrong_width
#   变异输入：initialStateOptional.present_shape = [1, 3, 1536]
#   违反约束：initial state must be B,W,D
# case 12: causal_conv1d_bwd_abnormal_dht_wrong_dim
#   变异输入：dhtOptional.present_shape = [1, 4, 1520]
#   违反约束：dht must be B,W,D
# case 13: causal_conv1d_bwd_abnormal_y_wrong_time
#   变异输入：yOptional.present_shape = [1, 32767, 1536]
#   违反约束：provided y must match output shape
# case 14: causal_conv1d_bwd_abnormal_dy_dtype_mismatch
#   变异输入：dy.dtype = "fp16"
#   违反约束：dy dtype must match x
@register("npu_causal_conv1d_bwd_abnormal")
class AbnormalReferenceApi(CausalConv1dBwdApi):
    ...
    def __call__(self, input_data: InputDataset, with_output: bool = False):
        ...
        return (torch.zeros((1, 32768, 1536), dtype=torch.bfloat16, device=device), torch.zeros((4, 1536), dtype=torch.bfloat16, device=device), torch.zeros((1536,), dtype=torch.bfloat16, device=device), torch.zeros((1, 4, 1536), dtype=torch.bfloat16, device=device),)
