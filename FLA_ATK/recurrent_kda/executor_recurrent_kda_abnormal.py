...
# 异常用例约束说明（对应 atk_recurrent_kda_abnormal.json）。
# 以下为各用例设计时违反的输入约束；固定输出标杆不执行约束校验。
# 实际 DUT 的拒绝行为及报错需由运行结果确认。
# case 00: recurrent_kda_abnormal_query_fp32
#   变异输入：query.dtype = "fp32"
#   违反约束：q/k/v only support BF16
# case 01: recurrent_kda_abnormal_key_dim_127
#   变异输入：key.shape = [1, 1, 96, 127]
#   违反约束：K must be 128 and match query
# case 02: recurrent_kda_abnormal_value_dim_127
#   变异输入：value.shape = [1, 1, 96, 127]
#   违反约束：V must be 128/256
# case 03: recurrent_kda_abnormal_query_rank_3
#   变异输入：query.shape = [1, 96, 128]
#   违反约束：BSND query must be rank 4
# case 04: recurrent_kda_abnormal_gate_head_mismatch
#   变异输入：gate.shape = [1, 1, 95, 128]
#   违反约束：gate heads must equal HV
# case 05: recurrent_kda_abnormal_beta_rank_2
#   变异输入：beta.shape = [1, 96]
#   违反约束：BSND beta must be rank 3
# case 06: recurrent_kda_abnormal_state_rank_3
#   变异输入：initialStateRef.shape = [96, 128, 128]
#   违反约束：state must be rank 4
# case 07: recurrent_kda_abnormal_state_key_127
#   变异输入：initialStateRef.shape = [1, 96, 127, 128]
#   违反约束：state K must match query K
# case 08: recurrent_kda_abnormal_layout_invalid
#   变异输入：layout.range_values = "INVALID"
#   违反约束：layout supports BSND/TND
# case 09: recurrent_kda_abnormal_accepted_zero
#   变异输入：numAcceptedTokensOptional.range_values = [0]
#   违反约束：accepted token count must be at least 1
# case 10: recurrent_kda_abnormal_accepted_too_large
#   变异输入：numAcceptedTokensOptional.range_values = [2]
#   违反约束：accepted token count exceeds decode length 1
# case 11: recurrent_kda_abnormal_index_out_of_pool
#   变异输入：ssmStateIndicesOptional.range_values = [1]
#   违反约束：slot 1 exceeds state capacity 1
# case 12: recurrent_kda_abnormal_cu_start_nonzero
#   变异输入：cuSeqlensOptional.range_values = [1, 1]
#   违反约束：cu_seqlens must start at 0
# case 13: recurrent_kda_abnormal_gate_without_alog
#   变异输入：useGateInKernel.range_values = true
#   违反约束：in-kernel gate requires A_log
# case 14: recurrent_kda_abnormal_gate_dtype_int32
#   变异输入：gate.dtype = "int32"
#   违反约束：gate only supports FP16/BF16/FP32
@register("npu_recurrent_kda_abnormal")
class AbnormalReferenceApi(FunctionApi):
    ...
    def __call__(self, input_data: InputDataset, with_output: bool = False):
        ...
        return (torch.zeros((1, 1, 96, 128), dtype=torch.bfloat16, device=device), torch.zeros((1, 96, 128, 128), dtype=torch.float32, device=device),)
